...
try:
    image_bolo = Image.open("imagens/1.jpeg")  # Substitua pelo nome da sua foto
    st.image(image_bolo, caption="Um bolo especial!", use_container_width=True) # ALTERAÇÃO AQUI
except FileNotFoundError:
    st.error("Erro: A imagem 'foto_bolo.jpg' não foi encontrada na pasta 'images'.")
...
cols = st.columns(2)  # Cria duas colunas

try:
    image_festa1 = Image.open("imagens/2.jpeg")
    cols[0].image(image_festa1, caption="Celebrando!", use_container_width=True) # ALTERAÇÃO AQUI
except FileNotFoundError:
    cols[0].error("Imagem 'foto_festa.jpeg' não encontrada.")

try:
    image_festa2 = Image.open("imagens/3.jpeg")  # Usando outra imagem como exemplo
    cols[1].image(image_festa2, caption="Mais festa!", use_container_width=True) # ALTERAÇÃO AQUI
except FileNotFoundError:
    cols[1].error("Imagem 'foto_bolo.jpg' não encontrada.")
